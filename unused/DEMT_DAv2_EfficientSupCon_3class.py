# """
# Mean Teacher + DAv2 with patch-wise 3-class SupCon.

# - Unlabeled: teacher at full image resolution; per-cell mean entropy gate, then pseudo-label / elimination.
# - Labeled: full-res GT per feature cell (aligned split); one embedding per cell F[:, :, i, j].
# - Per cell: fg fraction from the aligned image crop; eliminate lean mixes, then class 0 / 1 / 2.
# - SupCon on (N, C) with max_samples (default 512). Contrastive uses design A (no D×D pool on features).
# """
...
# from utils.loss import MSELoss, WeightedBCEDiceLoss, BCELoss, MemoryEfficientSupContrastive3ClassLoss
# from utils.contrastive_patchwise import build_patchwise_supcon
# import torch.nn.functional as F


# class DEMT_DAv2_EfficientSupCon3Class_Trainer(Trainer):
#     def __init__(
#         self,
#         stu_model,
#         tea_model,
...
#         contrastive_rampup_epochs=16.0,
...
#         elimination_lo=0.25,
#         elimination_hi=0.75,
#         unlabeled_patch_entropy_max=None,
#         **kwargs,
#     ) -> None:
...
#         self.contrastive_loss = MemoryEfficientSupContrastive3ClassLoss(
...
#         self.elimination_lo = float(elimination_lo)
#         self.elimination_hi = float(elimination_hi)
#         self.unlabeled_patch_entropy_max = unlabeled_patch_entropy_max
...
#                        'consistency_weight': [], 'contrastive_weight': [], 'unlabeled_rgbd_cutmix_loss': [],
...
#             uh, uw = int(unlabeled_img.shape[2]), int(unlabeled_img.shape[3])
#             feat_c, lab_c = build_patchwise_supcon(
#                 labeled_stu_feature,
#                 label,
#                 unlabeled_stu_feature,
#                 tea_output,
#                 patch_entropy_max=self.unlabeled_patch_entropy_max,
#                 elimination_lo=self.elimination_lo,
#                 elimination_hi=self.elimination_hi,
#                 unlabeled_image_hw=(uh, uw),
#             )
#             if feat_c.shape[0] < 2:
#                 contrastive_loss = labeled_stu_feature.sum() * 0.0
#             else:
#                 contrastive_loss = self.contrastive_loss(feat_c, lab_c)
...
#             consistency_weight = self._get_current_consistency_weight(global_step=global_step)
#             contrastive_weight = self._get_current_contrastive_weight(global_step=global_step)
#             total_loss = (
#                 loss_sup
...
#             phase1_info['contrastive_weight'].append(contrastive_weight)
...
#     fl = int(cfg.get('Trainer.feature_layers', cfg.get('model.stu_model.feature_layers', 2)))
#     add_ph = bool(cfg.get('Trainer.add_proj_head', cfg.get('model.stu_model.add_proj_head', True)))
...
#     trainer = DEMT_DAv2_EfficientSupCon3Class_Trainer(
...
#         max_samples=int(cfg.get('Trainer.max_samples', 512)),
#         contrastive_temperature=float(cfg.get('Trainer.contrastive_temperature', 0.1)),
#         feature_layers=fl,
#         elimination_lo=float(cfg.get('Trainer.elimination_lo', cfg.get('Trainer.boundary_lo', 0.25))),
#         elimination_hi=float(cfg.get('Trainer.elimination_hi', cfg.get('Trainer.boundary_hi', 0.75))),
#         unlabeled_patch_entropy_max=cfg.get('Trainer.unlabeled_patch_entropy_max', 0.2),
#     )
...
#         description='Mean Teacher + DAv2 with 3-class boundary-aware efficient supervised contrastive loss.'
#     )
#     parser.add_argument('--optuna_trial_times', type=int, default=4, help='Optuna trials; 0 = no Optuna.')
#     parser.add_argument('--config', type=str, default='cfg/DEMT_DAv2_EfficientSupCon_3class.yaml', help='Path to YAML config')
