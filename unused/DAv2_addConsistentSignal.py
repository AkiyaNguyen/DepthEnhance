...
#         result["tea_scheduler"] = (
#             self.tea_scheduler.state_dict()
#             if hasattr(self, "tea_scheduler") and self.tea_scheduler is not None
#             else None
#         )
#         return result
...
#             def feature_grid_confidence_mask(tea_prob, feat_hw):
#                 t = self.teacher_reliable_threshold
#                 threshold = t - 0.5
#                 dev = torch.abs(tea_prob - 0.5)
#                 pooled = torch.nn.functional.adaptive_avg_pool2d(dev, feat_hw)
#                 return (pooled > threshold).float()

#             mask_u = teacher_confidence_mask(tea_output)
#             loss_consist_rgbd = self.consistency_criterion(unlabeled_stu, tea_output, mask=mask_u)
...
#             )
#             total_loss = loss_sup + consistency_weight * loss_consist + loss_consist_rgbd_cutmix
...
#             phase1_info["unlabeled_feat_loss"].append(loss_feat.item())
...
#                 tea_unlabeled_rgbd_output, round_stu_target, mask=tea_learn_from_stu_mask
#             )
...
# class MeanTeacherEvalHook_DAv2_addConsistentSignal(EvalHook):
#     def __init__(
#         self, trainer: Trainer, eval_data_loader: torch.utils.data.DataLoader, eval_every_epoch: int, prefix: str = ""
#     ) -> None:
...
#     stu_model = getattr(models, cfg.get("model.stu_model.name"))(num_classes=cfg.get("model.num_channels_output")).to(
#         device
#     )
#     tea_model = getattr(models, cfg.get("model.tea_model.name"))(num_classes=cfg.get("model.num_channels_output")).to(
#         device
#     )
...
#         add_proj_head=add_proj,
#         middle_channels=middle_ch,
#         out_channels=proj_out_ch,
#     )
...
#     )
#     tea_optimizer = torch.optim.SGD(
#         tea_model.parameters(),
#         lr=cfg.get("tea_optimizer.lr", cfg.get("optimizer.lr")),
...
#     trainer = DAv2Fusion_MT_Trainer_addConsistentSignal(
#         stu_model,
#         tea_model,
#         train_dataloader,
#         optimizer,
...
#         consistency_rampup_epochs=float(cfg.get("Trainer.consistency_rampup_epochs", 100)),
...
#         hook_builder(
#             MeanTeacherEvalHook_DAv2_addConsistentSignal,
