# ## DAv2 adding boundary-guided contrastive learning
...
# from utils.contrastive_patchwise import patch_fold, average_entropy, classify_patch_type

# from utils.build_dataset import build_dataset
# from utils import contrastive_loss as contrastive_loss_mod
# from utils.loss import MSELoss, WeightedBCEDiceLoss, BCELoss
# import inspect


# class DAv2Fusion_MT_Trainer_BGCL(Trainer):
#     """
...
#                  student_reliable_threshold=0.85, depth_learn_from_stu_weight=1.0, low_bd_thresh=0.25, high_bd_thresh=0.75,
#                  prototype_nce_type='PrototypeNCE_FG_BG_BD', prototype_nce_temperature=0.1, prototype_nce_max_samples=256,
#                  contrastive_weight: float = 0.1, contrastive_rampup_epochs: int = 60,
#                  unlabeled_entropy_threshold: float = 0.2,
#                  **kwargs) -> None:
...
#         # self.low_bd_thresh = low_bd_thresh
#         # self.high_bd_thresh = high_bd_thresh
#         # self.prototype_nce_temperature = prototype_nce_temperature

#         args = inspect.signature(self.__init__).parameters
#         for name in args:
#             if name != 'self': # current not necessary
...
#         try:
#             self.contrastive_loss = getattr(contrastive_loss_mod, prototype_nce_type)(
#                 temperature=prototype_nce_temperature,
#                 max_samples=prototype_nce_max_samples,
#             )
#         except AttributeError:
#             raise ValueError(
#                 f"Invalid prototype NCE type: {prototype_nce_type}. "
#                 f"Available: {getattr(contrastive_loss_mod, '__all__', [])}"
#             )
#         print(f"Using {prototype_nce_type} for contrastive loss")
#         self.feature_layers = self.stu_model.feature_layers 
#         self.patch_size = 2 ** (self.feature_layers - 1)
...
#     def _get_current_contrastive_weight(self, global_step):
#         return self.contrastive_weight * sigmoid_rampup(current=global_step, rampup_length=self.contrastive_rampup)
...
#                        'contrastive_loss_fg': [], 'contrastive_loss_bg': [], 'contrastive_loss_bd': [], 'contrastive_loss': []}
...
#             labeled_stu, unlabeled_stu = stu_pred[:self.labeled_bs], stu_pred[self.labeled_bs:]
#             labeled_stu_fea, unlabeled_stu_fea = stu_fea[:self.labeled_bs], stu_fea[self.labeled_bs:]
...
#             patch_fold_labeled_stu = patch_fold(label, patch_size=self.patch_size) # shape (b * h' * w', p^2)
#             patch_fold_unlabeled_stu = patch_fold(unlabeled_stu, patch_size=self.patch_size) # shape (b_un * w' * h', p^2)
#             patch_fold_unlabeled_tea = patch_fold(tea_output, patch_size=self.patch_size) # shape (b_un * w' * h', p^2)

#             # Low entropy (high confidence) on student and teacher patch predictions.
#             stu_ent = average_entropy(patch_fold_unlabeled_stu)
#             tea_ent = average_entropy(patch_fold_unlabeled_tea)
#             certainty_mask = (stu_ent <= self.unlabeled_entropy_threshold) & (tea_ent <= self.unlabeled_entropy_threshold)

#             chosen_patch_fold_unlabeled_stu = patch_fold_unlabeled_stu[certainty_mask]

#             # Row order must match patch_fold: (b, h, w) raster → use NHWC then flatten, not NCHW.reshape(-1,C).
#             Bu, C_u, Hf_u, Wf_u = unlabeled_stu_fea.shape
#             unlabeled_fe_flat = unlabeled_stu_fea.permute(0, 2, 3, 1).reshape(-1, C_u)
#             chosen_unlabeled_stu_fea = unlabeled_fe_flat[certainty_mask]

#             Bl, C_l, Hf_l, Wf_l = labeled_stu_fea.shape
#             labeled_fe = labeled_stu_fea.permute(0, 2, 3, 1).reshape(-1, C_l)
#             n_lab_patch = patch_fold_labeled_stu.shape[0]
#             assert labeled_fe.shape[0] == n_lab_patch == Bl * Hf_l * Wf_l, (
#                 f"Labeled rows {labeled_fe.shape[0]} vs patches {n_lab_patch} vs Bl*Hf*Wf={Bl}*{Hf_l}*{Wf_l}; "
#                 "feature map must match patch grid (feature_layers / patch_size vs input H,W)."
#             )
#             assert unlabeled_fe_flat.shape[0] == patch_fold_unlabeled_stu.shape[0] == Bu * Hf_u * Wf_u, (
#                 f"Unlabeled feat rows {unlabeled_fe_flat.shape[0]} vs patch_fold rows {patch_fold_unlabeled_stu.shape[0]}"
#             )
#             assert certainty_mask.numel() == unlabeled_fe_flat.shape[0]

#             candidate_features = torch.cat([labeled_fe, chosen_unlabeled_stu_fea], dim=0)
#             candidate_labels = torch.cat([patch_fold_labeled_stu, chosen_patch_fold_unlabeled_stu], dim=0)

#             fg_mask, bg_mask, boundary_mask = classify_patch_type(candidate_labels, low_bd_thresh=self.low_bd_thresh, \
#                 high_bd_thresh=self.high_bd_thresh)

#             fg_fea, bg_fea, bd_fea = candidate_features[fg_mask], \
#                 candidate_features[bg_mask], candidate_features[boundary_mask]

#             contrastive_loss_info = self.contrastive_loss(fg_fea, bg_fea, bd_fea)
#             if isinstance(contrastive_loss_info, dict):
#                 contrastive_loss = contrastive_loss_info['loss']
#             else:
#                 contrastive_loss = contrastive_loss_info
            
#             current_contrastive_weight = self._get_current_contrastive_weight(global_step=global_step)
...
#             if isinstance(contrastive_loss_info, dict):
#                 for key, value in contrastive_loss_info.items():
#                     phase1_info['contrastive_' + key].append(value.item())
#             else:
#                 phase1_info['contrastive_loss'].append(contrastive_loss.item())
...
# class MeanTeacherEvalHook_DAv2_BGCL(EvalHook):
...
#     # set feature layers for stu_model for contrastive learning, currently for student only
#     stu_model.set_feature_layers(feature_layers=cfg.get('model.stu_model.feature_layers', 3), \
#         add_proj_head=cfg.get('model.stu_model.add_proj_head', True))
...
#     trainer = DAv2Fusion_MT_Trainer_BGCL(
...
#         low_bd_thresh=float(cfg.get('Trainer.low_bd_thresh', 0.25)),
#         high_bd_thresh=float(cfg.get('Trainer.high_bd_thresh', 0.75)),
#         prototype_nce_type=str(cfg.get('Trainer.prototype_nce_type', 'PrototypeNCE_FG_BG_BD')),
#         prototype_nce_temperature=float(cfg.get('Trainer.prototype_nce_temperature', 0.1)),
#         prototype_nce_max_samples=int(cfg.get('Trainer.prototype_nce_max_samples', 256)),
#         contrastive_weight=float(cfg.get('Trainer.contrastive_weight', 0.1)),
#         contrastive_rampup_epochs=int(cfg.get('Trainer.contrastive_rampup_epochs', 60)),
#         unlabeled_entropy_threshold=float(cfg.get('Trainer.unlabeled_entropy_threshold', 0.2)),
#     )
...
#         hook_builder(MeanTeacherEvalHook_DAv2_BGCL, eval_data_loader=val_dataloader,
#                      eval_every_epoch=int(cfg.get('Hook.MeanTeacherEvalHook.eval_every_epoch')), prefix='val_')
#     hook_builder(MeanTeacherEvalHook_DAv2_BGCL, eval_data_loader=test_dataloader,
...
#     parser.add_argument('--config', type=str, default='cfg/DAv2_BGCL.yaml', help='Path to YAML config')
