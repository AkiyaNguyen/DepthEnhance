...
from BiFusion_DEMT_DAv2 import MeanTeacherEvalHook_DAv2_addDepthTrainSignal, BiFusion_DEMT_DAv2_Trainer


class BiFusion_DEMT_DAv2_TeaEval_Trainer(BiFusion_DEMT_DAv2_Trainer):
    """
    Same as BiFusion_DEMT_DAv2 trainer, except teacher uses eval() in phase 1
    pseudo-label forward for more stable student targets.
    """
    ...
    def run_step_(self) -> None:
        ...
        for batch_id, data in enumerate(self.train_dataloader):
            ...
            # TeaEval-only change: deterministic pseudo-label teacher in phase 1.
            self.tea_model.eval()
            with torch.no_grad():
            ...
            loss_consist_rgbd = self.consistency_criterion(
                unlabeled_stu, tea_output, mask=teacher_confidence_mask(tea_output)
            )
...
def training(cfg: Config, trial: typing.Optional[optuna.trial.Trial] = None):
    ...
    trainer = BiFusion_DEMT_DAv2_TeaEval_Trainer(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='BiFusion DEMT DAv2 with teacher eval() in phase 1.')
    parser.add_argument('--optuna_trial_times', type=int, default=4, help='Optuna trials; 0 = no Optuna.')
    parser.add_argument('--config', type=str, default='cfg/BiFusion_DEMT_DAv2_TeaEval.yaml', help='Path to YAML config')
...
#     python BiFusion_DEMT_DAv2_TeaEval.py \
...
#                     tea_optimizer.lr=0.005\
#                     Hook.ExtendMLFlowLoggerHook.run_name='BiFusion_DEMT_DAv2_TeaEval' \
