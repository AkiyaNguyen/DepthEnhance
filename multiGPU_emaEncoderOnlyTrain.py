...
class DepthEnhance_MT_Trainer_EMAEncoderOnly(Trainer):
    ...
    def __init__(self, stu_model, tea_model, train_dataloader, stu_optimizer, tea_optimizer, scheduler, num_epochs, ema_alpha,
                 iters_per_epoch: int, consistency_rampup_epochs, consistency, fea_sim_weight: float, tea_scheduler=None, **kwargs) -> None:
    ...
    def _update_ema_variable(self, global_step, model_a: nn.Module, model_b: nn.Module):
        # model_a, model_b are already raw submodules (after unwrap + submodule access)
    ...
    def run_step_(self) -> None:
        ...
        for batch_id, data in enumerate(self.train_dataloader):
            ...
            # accelerator.prepare handles device placement automatically
            ...
            # FIX: unwrap full model first, then access submodule
            tea_unwrapped = self.accelerator.unwrap_model(self.tea_model)
            stu_unwrapped = self.accelerator.unwrap_model(self.stu_model)
            self._update_ema_variable(
                global_step=batch_id + self.current_epoch * len(self.train_dataloader),
                model_a=tea_unwrapped.rgb_encoder,
                model_b=stu_unwrapped.encoder1,
            )
        ...
        for batch_id, data in enumerate(self.train_dataloader):
            ...
            # FIX: unwrap to freeze, but forward through self.tea_model (wrapped) for correct device handling
            tea_unwrapped = self.accelerator.unwrap_model(self.tea_model)
            for param in tea_unwrapped.rgb_encoder.parameters():
                param.requires_grad_(False)

            # FIX: forward through self.tea_model, not tea_unwrapped
            ...
            self.accelerator.backward(loss_tea_sup)
            ...
            # Unfreeze rgb_encoder for next iteration
            for param in tea_unwrapped.parameters():
...
def training(cfg: Config, trial: typing.Optional[optuna.trial.Trial] = None):
    ...
    tea_model = getattr(models, cfg.get('model.tea_model.name'))(num_classes=cfg.get('model.num_channels_output'))
    ...
    tea_depth_branch_optimizer = torch.optim.SGD(tea_model.parameters(), lr=cfg.get('tea_optimizer.lr', cfg.get('optimizer.lr')),
    ...
    tea_depth_branch_scheduler = CosineAnnealingLR(tea_depth_branch_optimizer, T_max=nEpoch, eta_min=eta_min)

    accelerator = Accelerator()

    stu_model, tea_model, \
    optimizer, tea_depth_branch_optimizer, \
    scheduler, tea_depth_branch_scheduler, \
    ...
    trainer.prepare_accelerate(accelerator)
