...
class DAv2_BARD_Trainer(Trainer):
    ...
    Inherits from Trainer base class with focus on spatial relation alignment.
    """
    ...
    def __init__(self, stu_model, tea_model, train_dataloader, stu_optimizer, tea_optimizer, scheduler, num_epochs, ema_alpha,
                 ...
                 student_reliable_threshold=0.85, depth_learn_from_stu_weight=1.0, relational_weight=0.1, **kwargs) -> None:
        ...
        self.stu_bottleneck_feat = None
        self.tea_dav2_bottleneck_feat = None
        
        # Register forward hooks to capture bottleneck features
        self._register_student_hook()
        self._register_teacher_hook()
        self._freeze_dav2_backbone()

    def _register_student_hook(self) -> None:
        """Register forward hook on student's deepest encoder layer to capture bottleneck features."""
        # Hook into the deepest encoder layer (encoder1 for ResNet34U_f)
        if hasattr(self.stu_model, 'encoder1'):
            def hook_fn(module, input, output):
                self.stu_bottleneck_feat = output
            self.stu_model.encoder1.register_forward_hook(hook_fn)

    def _register_teacher_hook(self) -> None:
        """Register forward hook on teacher's projected depth features to capture spatial features."""
        # For DAv2-based teachers, hook into proj5 which outputs (B, 512, H, W) spatial features
        if hasattr(self.tea_model, 'proj5'):
            def hook_fn(module, input, output):
                self.tea_dav2_bottleneck_feat = output
            self.tea_model.proj5.register_forward_hook(hook_fn)
        elif hasattr(self.tea_model, 'dav2_encoder'):
            # Fallback: try to hook dav2_encoder and extract features later
            def hook_fn(module, input, output):
                self.tea_dav2_bottleneck_feat = output
            self.tea_model.dav2_encoder.register_forward_hook(hook_fn)
    ...
    def _compute_spatial_relation(self, feat: torch.Tensor) -> torch.Tensor:
        """
        Compute spatial relation matrix from feature map.
        
        Args:
            feat: Feature map of shape (B, C, H, W)
            
        Returns:
            Spatial relation matrix of shape (B, N, N) where N = H*W
        """
        B, C, H, W = feat.shape
        
        # Normalize along channel dimension (L2 norm)
        feat_normalized = F.normalize(feat, p=2, dim=1)
        
        # Flatten spatial dimensions: (B, C, H, W) -> (B, C, N)
        feat_flat = feat_normalized.view(B, C, -1)
        
        # Compute spatial correlation matrix: (B, C, N) -> (B, N, N)
        # relation[b, i, j] = sum_c(feat_flat[b, c, i] * feat_flat[b, c, j])
        relation = torch.bmm(feat_flat.transpose(1, 2), feat_flat)
    ...
    def run_step_(self) -> None:
        ...
        for batch_id, data in enumerate(self.train_dataloader):
            ...
            # Forward student model and capture bottleneck features via hook
            stu_pred = self.stu_model(img_s)
            stu_feat = self.stu_bottleneck_feat  # Retrieved from forward hook
            
            # Extract tensor from tuple if necessary
            if isinstance(stu_feat, (tuple, list)):
                stu_feat = stu_feat[-1]
            ...
            phase1_info['relational_loss'].append(relational_loss.item())
...
def training_bard(cfg: Config, trial: typing.Optional[optuna.trial.Trial] = None):
    ...
    Returns:
        Score based on the configured criteria
    """
    ...
    trainer = DAv2_BARD_Trainer(
        ...
        relational_weight=float(cfg.get('Trainer.relational_weight', 0.1)),
    )
...
#     python DEMT_DAv2_Relational.py \
...
#                     Hook.ExtendMLFlowLoggerHook.run_name='DEMT_DAv2_Relational' \
#                     Hook.ExtendMLFlowLoggerHook.experiment_name='DEMT_DAv2_Relational' \
#                     Hook.ExtendMLFlowLoggerHook.meta_info.kaggle_run_link='https://www.kaggle.com/code/relational_distillation_polyp' \
