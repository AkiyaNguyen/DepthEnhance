...
class DEMT_DAv2_CL1_Trainer(Trainer):
    """
    Same as DEMT_DAv2_CL but uses MemoryEfficientSupContrastiveLoss (subsampled pixels per batch).
    """
    ...
    @staticmethod
    def _downsample_label(label, target_size):
        """
        label: (B, 1, H, W) float 0/1
        return: (B, 1, H', W') with values 0 / 1 / -1 (ignore)
        """
        H_orig, W_orig = label.shape[2:]
        H_t, W_t = target_size
        kH, kW = H_orig // H_t, W_orig // W_t

        avg = F.avg_pool2d(label, kernel_size=(kH, kW), stride=(kH, kW))

        result = torch.full_like(avg, fill_value=-1)
        result[avg > 0.7] = 1.0
        result[avg < 0.3] = 0.0
    ...
    def run_step_(self) -> None:
        ...
        for batch_id, data in enumerate(self.train_dataloader):
            ...
            labeled_resized = self._downsample_label(label, labeled_stu_feature.shape[2:])
            contrastive_loss = self.contrastive_loss(labeled_stu_feature, labeled_resized, ignore_label=-1)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Mean Teacher + DAv2 fusion with memory-efficient supervised contrastive loss (DEMT_DAv2_CL1).'
    )
    parser.add_argument('--optuna_trial_times', type=int, default=4, help='Optuna trials; 0 = no Optuna.')
    parser.add_argument('--config', type=str, default='cfg/DEMT_DAv2_CL1.yaml', help='Path to YAML config')
...
#                     Hook.ExtendMLFlowLoggerHook.run_name='DEMT_DAv2_CL1' \
#                     Hook.ExtendMLFlowLoggerHook.experiment_name='DEMT_DAv2_CL1' \
