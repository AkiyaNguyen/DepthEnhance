...
class DEMT_DAv2_CL_Trainer(Trainer):
    ...
    def _downsample_label(label, target_size):
        ...
        avg = F.avg_pool2d(label, kernel_size=(kH, kW), stride=(kH, kW)) # (B, 1, H', W')

        result = torch.full_like(avg, fill_value=-1)# default: ignore
        result[avg > 0.7] = 1.0 # foreground
        result[avg < 0.3] = 0.0 # background
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Mean Teacher + DAv2 fusion with feature-level contrastive loss (DEMT_DAv2_CL).'
    )
    parser.add_argument('--optuna_trial_times', type=int, default=4, help='Optuna trials; 0 = no Optuna.')
    parser.add_argument('--config', type=str, default='cfg/DEMT_DAv2_CL.yaml', help='Path to YAML config')
...
#                     Hook.ExtendMLFlowLoggerHook.run_name='DEMT_DAv2_CL' \
#                     Hook.ExtendMLFlowLoggerHook.experiment_name='DEMT_DAv2_CL' \
